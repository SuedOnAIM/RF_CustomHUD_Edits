...
behaviour("CustomHUD_BalanceBarMC")

function CustomHUD_BalanceBarMC:Start()
...
function CustomHUD_BalanceBarMC:monitorHUDVisibility()
	return PlayerHud.hudPlayerEnabled
end

function CustomHUD_BalanceBarMC:onHUDVisibilityChange()
	self.targets.Canvas.enabled = not Player.actor.isDead and GameManager.hudPlayerEnabled and self.balanceBarVisibility
end

function CustomHUD_BalanceBarMC:monitorCurrentBalance()
	return Player.actor.balance
end

function CustomHUD_BalanceBarMC:monitorCurrentMaxBalance()
	return Player.actor.maxBalance
end

function CustomHUD_BalanceBarMC:updateBalanceUI()
...
function CustomHUD_BalanceBarMC:onActorSpawn(actor)
...
function CustomHUD_BalanceBarMC:onActorDied(actor,source,isSilent)
	if actor.isPlayer then
		self.targets.Canvas.enabled = false
	end
end