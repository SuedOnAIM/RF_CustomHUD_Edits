...
behaviour("CustomHUD_SimpleGameFeedMC")

function CustomHUD_SimpleGameFeedMC:Start()
...
function CustomHUD_SimpleGameFeedMC:monitorHUDVisibility()
	return GameManager.hudPlayerEnabled
end

function CustomHUD_SimpleGameFeedMC:onHUDVisibilityChange()
	self.targets.Canvas.enabled = GameManager.hudPlayerEnabled and self.feedEnabled
end

function CustomHUD_SimpleGameFeedMC:Update()
...
function CustomHUD_SimpleGameFeedMC:Clear()
	for i = 1, self.maxLines, 1 do
		self.lines[i] = ""
	end
	self.feed.text = ""
...
function CustomHUD_SimpleGameFeedMC:UpdateFeed()
	local text = ""
...
function CustomHUD_SimpleGameFeedMC:PushText(text)
	for i=1,self.maxLines-1 do
...
function CustomHUD_SimpleGameFeedMC:onActorDied(actor, source, isSilent)
	...
	if source then
		...
		if source.activeWeapon then
			...
			if source.activeWeapon.weaponEntry then
				...
				weaponName = self:CleanString(weaponName,"TACTICAL", " TACTICAL")
				weaponName = self:CleanString(weaponName,"Taktyčny", " Taktyčny")
				weaponName = self:CleanString(weaponName,"Taktički", " Taktički")
			...
			self:PushText(sourceName .." killed " .. actorName .. " with " .. "".. sourceWeapon.."")
		else
...
function CustomHUD_SimpleGameFeedMC:CleanString(str, target, format)
...
function CustomHUD_SimpleGameFeedMC:onCapturePointCaptured(capturePoint, newOwner)
...
function CustomHUD_SimpleGameFeedMC:onCapturePointNeutralized(capturePoint, previousOwner)
	if self.hasSpawnedOnce then
		local capturePointText = capturePoint.name
...
function CustomHUD_SimpleGameFeedMC:onActorSpawn(actor)
