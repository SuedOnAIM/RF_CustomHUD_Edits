...
behaviour("CustomHUD_HealthBarMC")

function CustomHUD_HealthBarMC:Start()
...
function CustomHUD_HealthBarMC:monitorHUDVisibility()
	return PlayerHud.hudPlayerEnabled
end

function CustomHUD_HealthBarMC:onHUDVisibilityChange()
	self.targets.Canvas.enabled = not Player.actor.isDead and GameManager.hudPlayerEnabled and self.healthBarVisibility
end

function CustomHUD_HealthBarMC:monitorCurrentHealth()
	return Player.actor.health
end

function CustomHUD_HealthBarMC:monitorCurrentMaxHealth()
	return Player.actor.maxHealth
end

function CustomHUD_HealthBarMC:monitorEHSMaxHealth()
	return self.enhancedHealth.self.maxHP
end

function CustomHUD_HealthBarMC:monitorEHSOverHeal()
	return self.enhancedHealth.self.overHealMax
end

function CustomHUD_HealthBarMC:updateHealthUI()
...
function CustomHUD_HealthBarMC:updateHealthUIEnhanced()
	if Player.actor then
		if self.targets.healthBar then
...
function CustomHUD_HealthBarMC:updateOverHealBar()
	if Player.actor then
...
function CustomHUD_HealthBarMC:onActorSpawn(actor)
...
function CustomHUD_HealthBarMC:onActorDied(actor,source,isSilent)
	if actor.isPlayer then
		self.targets.Canvas.enabled = false
	end
end