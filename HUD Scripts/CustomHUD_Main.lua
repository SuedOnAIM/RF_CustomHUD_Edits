...
function CustomHUD_Main:Start()
	...
	if GameManager.buildNumber < 26 then
		...
		print("<color=red>[Custom HUD]Using an older version below EA26 may cause issues</color>")
	end
...
function CustomHUD_Main:DisableDefaultHUD()
	...
	--[[even more hud disabled
	if not self.script.mutator.GetConfigurationBool("killIndicatorVisibility") then
		...
		GameObject.Find("Hitmarker").gameObject.SetActive(false)
		end--]]
end